...
function ScheduleLabel({ s }: { s: AgentSummary['schedule'] }): JSX.Element {
  ...
  return <span className="text-ink-500">manual</span>;
}

function LastRunCell({ run }: { run: AgentSummary['lastRun'] }): JSX.Element {
  if (!run) return <span className="text-ink-400">—</span>;
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="truncate text-xs text-ink-500">{run.summary ?? ''}</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="px-8 py-6">
        {error && (
          <div className="card mb-4 border-red-200 bg-red-50 p-4 text-sm text-red-800">
            {error}
          </div>
        )}
        {!agents ? (
          ...
          <div className="card overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ink-100">
                {agents.map((a) => (
                  <tr key={a.slug} className="hover:bg-ink-50">
